...
const Home = () => {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center px-6 bg-gradient-to-br from-blue-50 via-slate-50 to-white relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 left-10 w-64 h-64 rounded-full bg-blue-300 blur-3xl"></div>
        <div className="absolute bottom-20 right-10 w-64 h-64 rounded-full bg-indigo-300 blur-3xl"></div>
      </div>

      <div className="max-w-4xl mx-auto text-center relative z-10">
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <p className="inline-block mb-4 px-4 py-1 bg-blue-100 text-blue-600 rounded-full text-sm font-medium">
          ...
          <div className="flex flex-wrap gap-4 justify-center items-center mb-12">
            <div className="flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full">
              <Coffee size={18} className="text-slate-700" />
              <span className="text-slate-700">Coffee Enthusiast</span>
            </div>
            <div className="flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full">
              <Trophy size={18} className="text-slate-700" /> {/* Replaced Cricket with Trophy */}
              <span className="text-slate-700">Cricket Fan</span>
            </div>
            <div className="flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full">
              <Code size={18} className="text-slate-700" />
              <span className="text-slate-700">Problem Solver</span>
            </div>
          </div>
